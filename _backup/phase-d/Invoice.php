...
class Invoice extends Model
{
    ...
    {
        $cashAllocated = (float) $this->allocations()->sum('amount_allocated');
        $creditApplied = (float) \App\Models\CreditBalanceUsage::where('invoice_id', $this->id)
            ->where('type', 'DEBIT')
            ->sum('amount');

        return $cashAllocated + $creditApplied;
    }
