...
class DsiImporterService
{
    ...
    private function importRow(array $row, int $batchId, int $importedBy): void
    {
        ...
        // HARD GUARD: Layer 2 (REF_NO) must be rejected immediately to prevent re-import.
        if ($flagData && $flagData['detection_layer'] === 'REF_NO') {
            throw new \RuntimeException($flagData['detection_reason']);
        }
        ...
        // Layer 3 (SEMANTIC) is just a flag for human review.
        if ($flagData) {
