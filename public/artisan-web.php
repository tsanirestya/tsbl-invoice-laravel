...
// ── Whitelist ──────────────────────────────────────────────────────────────
// Only safe, idempotent production commands allowed.
$allowed = [
    'migrate'                => ['--force'],
    'migrate:status'         => [],
    'cache:clear'            => [],
...
if ($cmd !== '' && isset($allowed[$cmd])) {
    $args = array_merge([$cmd], $allowed[$cmd]);

    ob_start();
    $status = \Illuminate\Support\Facades\Artisan::call($cmd, array_combine(
        $allowed[$cmd],
        array_fill(0, count($allowed[$cmd]), true)
    ));
...
<style>
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .btn { padding: .55rem .75rem; border-radius: 8px; border: 1px solid #334155; background: #0f172a; color: #cbd5e1; font-size: .8rem; font-weight: 600; cursor: pointer; text-align: left; transition: background .15s, border-color .15s; display: flex; align-items: center; gap: .5rem; }
    ...
    <div class="btn-grid">
      <?php foreach (array_keys($allowed) as $c): ?>
        <form method="POST">
          ...
          <button type="submit" class="btn <?= str_contains($c, 'migrate') ? 'btn-danger' : '' ?>" style="width:100%">
